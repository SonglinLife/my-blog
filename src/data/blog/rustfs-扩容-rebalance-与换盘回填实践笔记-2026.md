...
本文沿一个本地实验走完整条路径：先从 4 node / 8 disk 写入对象，再新增第二个 pool，启动 post-expansion rebalance，观察对象文件怎么流向新 pool；最后模拟一块盘损坏并换空盘，看 RustFS 如何把缺失 shard 回填回来。
...
因此本实验看到的回填更像节点重新加入后触发的自动修复或 read repair，而不是手动 heal 命令的结果。这里不把它扩展成完整 heal 状态机结论，只记录实验事实：
