...
- [4 pool.bin：pool 生命周期账本，不是对象索引](#4-poolbinpool-生命周期账本不是对象索引)
- [5 PUT 2 MiB：对象如何进入 set 并落盘](#5-put-2-mib对象如何进入-set-并落盘)
...
说明 decommission 路径会保存 `pool_meta` 并通知 reload；
...
说明 peer reload 会重新加载 `PoolMeta` 并替换内存状态。
