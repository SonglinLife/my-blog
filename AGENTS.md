...
- Paper-style reference: in technical deep-dives, prefer `本文`, `本节`, `本实验`, `本环境`, and `本结论` over conversational phrases such as `这篇`, `这次`, `我们这次`, or `我这次`. Keep the article voice close to a technical report while still readable.
- Anti-AI texture: avoid uniformly templated paragraphs such as purpose preface -> command -> result -> interpretation repeated in every section. Remove redundant intention labels like `这里要验证的是` when the command and surrounding prose already make the purpose clear. Let paragraph length vary naturally; some evidence blocks can stand with a short lead-in, while surprising results deserve more explanation.
- Captions: do not make figure captions a plain restatement of the image. Use captions to point at the non-obvious value, boundary, caveat, or comparison the reader should notice.
- Conclusions: avoid symmetrical slogan endings unless the system is truly symmetrical. Prefer a mechanism conclusion that preserves the actual unevenness of the experiment, including partial migrations, failed assumptions, or boundaries that do not line up cleanly.
...
- `.agents/skills/blog-architecture-diagrams/`: project skill and renderer for precise editable architecture diagrams (canonical source; `.claude/skills/` and `.codex/skills/` are symlinks to it).
